...
| M19–M22 到货至仓库 | 未完成 | 下一步从 `tasks/M19_到货.md` 开始；按 M19→M22 逐项实现 |
...
1. **下一模块：M19 到货**，入口 `tasks/M19_到货.md`、`docs/07_商品采购仓储库存设计.md`。M19 需累计采购明细已收数量并更新 `PART_RECEIVED` / `RECEIVED`，再进入 M20 质检。若开发目标转为让小程序可见，应另开明确任务实现 uni-app 运行骨架和首页，再接商品 HTTP 路由/网络适配。
