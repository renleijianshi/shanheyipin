...
| M20–M22 质检至仓库 | 未完成 | 下一步从 `tasks/M20_质检.md` 开始；按 M20→M22 逐项实现 |
...
1. **下一模块：M20 质检**，入口 `tasks/M20_质检.md`、`docs/07_商品采购仓储库存设计.md`。质检需记录检查项、抽检图片、合格/降级/部分拒收/全部拒收结论，并保留收货记录与库存批次之间的边界。若开发目标转为让小程序可见，应另开明确任务实现 uni-app 运行骨架和首页，再接商品 HTTP 路由/网络适配。
