...
- D025：支付回调以 Provider 验证器作为不可信输入边界；验签后才解析，平台事件 ID 与交易 ID 双重唯一，支付单、订单、金额与状态日志在同一数据库事务更新。真实微信验证器缺失时保持关闭。
- D026：库存余额必须满足物理库存等于六类状态库存之和；余额更新与带来源单据的流水在同一行锁事务完成，余额不得绕过流水直接修改。
- D027：退款单与售后单分离，累计成功退款不得超过订单实付；真实退款 Provider 保持 Deferred。补发使用独立补发单，并在创建时原子扣减批次库存、记录出库流水与成本。
